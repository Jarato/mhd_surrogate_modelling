...
# Try importing pyvista, warn if missing
try:
...
def _render_3d_frame_worker(args_tuple, common_args):
    ...
    # Create PyVista Grid
    # PyVista RectilinearGrid takes x, y, z arrays defining the grid lines
    ...
    # --- Rendering Style ---
    # We use volume rendering with opacity. 
    # 'linear' opacity means low values are transparent, high values are opaque.
    plotter.add_volume(
        grid, 
        scalars="values", 
        cmap=common_args['cmap'], 
        opacity="linear", # or 'sigmoid' or a custom list
    ...
    # Camera setup (Isometric view by default)
    plotter.view_isometric()
    
    # Optional: Force exact camera position if needed
...
def generate_3d_video(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Parallel Processing
        # Note: 3D Rendering is GPU/CPU heavy. Too many workers might crash VRAM.
        # Safe default is often lower than CPU count.
