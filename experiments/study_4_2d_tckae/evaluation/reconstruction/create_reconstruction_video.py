# -*- coding: utf-8 -*-
# scripts/create_reconstruction_video.py
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate a 3-panel comparison video (Ground Truth, Reconstruction, Difference) from .npz files.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )

    # --- I/O Arguments ---
    parser.add_argument("--ground-truth-npz", type=Path, required=True, help="Path to the ground truth timeseries .npz file.")
    parser.add_argument("--reconstructed-npz", type=Path, required=True, help="Path to the reconstructed timeseries .npz file.")
    ...
    main_map.add_argument("--cmap", type=str, default="seismic", help="Colormap for Ground Truth and Reconstruction.")
    ...
    generate_comparison_video_from_npz(
        ...
        pred_npz_path=args.reconstructed_npz, # <-- Changed from predicted_npz
...
# Run the command to generate a reconstruction video for the 'u' (vx) component
python create_reconstruction_video.py \
    ...
    --reconstructed-npz "${EVAL_DIR}/reconstructed_timeseries.npz" \
    --difference-npz "${EVAL_DIR}/difference_timeseries.npz" \
    --output-path "${EVAL_DIR}/reconstruction_videos/reconstruction_u.mp4" \
    --channel "vx" \
    --channel-alias "u" \
    --fps 2 \
    --num-workers 128 \
