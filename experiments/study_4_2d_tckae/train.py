# -*- coding: utf-8 -*-
# experiments/study_3_q2d_tckae/train_2d_structured.py
# Note: This is a refactored version of the training script for improved clarity.
# The core functionality remains identical to the original script.
...
def compute_loss_tckae(
    ...
    """Computes the combined tcKAE loss. Unchanged from original."""
    loss_fn = nn.MSELoss()
...
def validate_epoch_rollout(model: tcKoopmanAutoencoder2D, dataloader: DataLoader, rollout_steps: int) -> Dict[str, float]:
    """Performs auto-regressive rollout validation. Unchanged from original."""
    model.eval()
    loss_fn = nn.MSELoss()
...
def find_latest_checkpoint(persistent_dir: Path, scratch_dir: Path | None) -> Path | None:
    """Finds the most recent checkpoint file. Unchanged from original."""
...
def main():
    ...
    1. Parse arguments.
    2. Set up environment (directories, logging).
    3. Load data and create dataloaders.
    4. Initialize model, optimizer, etc., either from scratch or a checkpoint.
    5. Run the main training loop.
    6. Log hyperparameters and results.
    """
    args = parse_args()
    ...
    # This dictionary holds all assets needed for the training loop.
    training_state = {}
    ...
    if resume_checkpoint_path:
        ...
        # Re-create args from checkpoint config to ensure consistency
        # This prevents breaking a resumed run with conflicting CLI args
        ...
        # Re-create dataloaders with the exact same data split and channels
    ...
    # Combine static data assets with the loaded assets
