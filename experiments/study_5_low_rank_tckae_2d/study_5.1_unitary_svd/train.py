...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train a Temporally-Consistent 2D Koopman Autoencoder (SVD Version)."
    )
    ...
    # --- MODIFIED ARGUMENTS (Replaced gamma_con) ---
    loss_group.add_argument("--gamma-svd-ortho", type=float, default=1e-4, help="Weight for SVD orthonormality loss (U^T*U=I, V^T*V=I).")
    loss_group.add_argument("--gamma-svd-sigma", type=float, default=1e-4, help="Weight for SVD singular value loss (Sigma=I).")
...
def compute_loss_tckae(
    ...
    --- MODIFIED ---
    Replaced the old consistency loss with SVD-based
    orthonormality and singular value losses.
    """
    loss_fn = nn.MSELoss()
    ...
    loss_svd_sigma = torch.tensor(0.0, device=DEVICE)
    ...
    if gammas["svd_sigma"] > 0:
        Sigma_vec = model.Sigma_learn
        # Target is a vector of ones
        ones_r = torch.ones_like(Sigma_vec)
        # L_singular_values = || Sigma_vec - 1 ||^2
        loss_svd_sigma = loss_fn(Sigma_vec, ones_r)
    # --- END SVD LOSSES ---
    ...
    total_loss = (
        ...
        gammas["tc"] * loss_tc + 
        gammas["bwd"] * loss_bwd + 
        gammas["svd_ortho"] * loss_svd_ortho +  # <-- NEW
        gammas["svd_sigma"] * loss_svd_sigma    # <-- NEW
    )
    ...
    loss_dict = {
        ...
        "svd_ortho": loss_svd_ortho.detach(),  # <-- NEW
        "svd_sigma": loss_svd_sigma.detach()   # <-- NEW
    }
...
def run_training_loop(
    ...
    gammas = {
        ...
        "svd_sigma": args.gamma_svd_sigma
    }
    ...
    for epoch in range(training_state["start_epoch"], args.epochs):
        ...
        epoch_losses = {
            ...
            "tc": 0.0, "backward": 0.0, "svd_ortho": 0.0, "svd_sigma": 0.0
        }
        epoch_grad_norm = 0.0
...
def main():
    ...
    # --- Finalize and Log HParams (MODIFIED) ---
