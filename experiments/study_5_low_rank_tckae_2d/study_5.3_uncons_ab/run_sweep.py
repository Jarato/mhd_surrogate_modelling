...
param_grid = {
    ...
    'gamma_tc': [0],
    'gamma_bwd': [0],
    'gamma_con': [0], # <-- ADDED (set to 0 as backward is False)
    'backward': [False],
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run a hyperparameter sweep for tcKAE (AB Decomposition Version).")
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    logging.info("Starting tcKAE (AB Decomposition) hyperparameter sweep...")
    ...
    for i, config in enumerate(run_configs):
        ...
        # --- Create a descriptive run name (MODIFIED for AB) ---
        run_name_parts = []
        ...
        run_name_parts.extend([
            ...
            f"g_con{config['gamma_con']}", # <-- MODIFIED
        ])
