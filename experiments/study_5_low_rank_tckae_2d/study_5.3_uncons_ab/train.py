...
#
# --- MODIFICATION ---
# Re-introduced the 'gamma_con' consistency loss to enforce
# K_fwd * K_bwd = I.
...
# --- MODIFIED: Import from the new uncons_ab package ---
from low_rank_tckae_2d_uncons_ab.data import tcKAEMHDDataset2D, RolloutMHDDataset2D
from low_rank_tckae_2d_uncons_ab.model import tcKoopmanAutoencoder2D
# --- END MODIFICATION ---
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train a Temporally-Consistent 2D Koopman Autoencoder (AB-Decomposition Version)."
    )
    ...
    model_group.add_argument("--koopman-rank", type=int, default=None, help="Rank 'r' for the AB Koopman operator. If not set, defaults to --latent-dim (if using bottleneck) or fails otherwise.")
    ...
    # --- MODIFICATION: Re-introduced gamma_con ---
    loss_group.add_argument("--gamma-con", type=float, default=1e-4, help="Weight for the backward/forward consistency loss (K * K_inv = I).")
    # --- END MODIFICATION ---
...
def compute_loss_tckae(
    ...
    --- MODIFIED ---
    Re-introduced consistency loss (gamma_con) for AB/CD operators.
    """
    loss_fn = nn.MSELoss()
    ...
    loss_consist = torch.tensor(0.0, device=DEVICE) # <-- MODIFIED
    
    if gammas["bwd"] > 0 or gammas["con"] > 0: # <-- MODIFIED
        ...
        # --- NEW CONSISTENCY LOSS (AB/CD version) ---
        if gammas["con"] > 0:
            # K_fwd = A @ B
            K_fwd = model.A_learn @ model.B_learn
            # K_bwd = C @ D
            K_bwd = model.C_learn @ model.D_learn
            
            # Identity matrix of size d x d
            I_d = torch.eye(model.latent_dim, device=K_fwd.device)
            
            # L_con = ||K_fwd @ K_bwd - I||^2 + ||K_bwd @ K_fwd - I||^2
            loss_con_1 = loss_fn(K_fwd @ K_bwd, I_d)
            loss_con_2 = loss_fn(K_bwd @ K_fwd, I_d)
            loss_consist = loss_con_1 + loss_con_2
        # --- END NEW CONSISTENCY LOSS ---
    ...
    total_loss = (
        ...
        gammas["tc"] * loss_tc + 
        gammas["bwd"] * loss_bwd +
        gammas["con"] * loss_consist # <-- MODIFIED
    )
    ...
    loss_dict = {
        ...
        "consistency": loss_consist.detach(), # <-- MODIFIED
    }
...
def run_training_loop(
    ...
    # --- MODIFIED: gammas dict (added 'con') ---
    gammas = {
        "identity": args.gamma_identity, 
        "fwd": args.gamma_fwd, 
        "bwd": args.gamma_bwd if args.backward else 0.0, 
        "tc": args.gamma_tc,
        "con": args.gamma_con if args.backward else 0.0, # <-- MODIFIED
    }
    ...
    for epoch in range(training_state["start_epoch"], args.epochs):
        ...
        # --- MODIFIED: epoch_losses dict (added 'consistency') ---
        epoch_losses = {
            "total": 0.0, "identity": 0.0, "forward": 0.0, 
            "tc": 0.0, "backward": 0.0, "consistency": 0.0
        }
        epoch_grad_norm = 0.0
        ...
        if avg_val_losses["total"] < best_val_loss:
            ...
            torch.save(best_model_data, persistent_dir / "best_model.pth")
...
def main():
    ...
    if resume_checkpoint_path:
        ...
        args.use_flattened_for_koopman = model_config.get("use_flattened_for_koopman", False)
        args.use_bottleneck = model_config.get("use_bottleneck", True)
        args.latent_dim = model_config.get("latent_dim")
        args.bottleneck_dim = model_config.get("bottleneck_dim")
