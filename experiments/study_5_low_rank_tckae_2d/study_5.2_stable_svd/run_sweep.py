# -*- coding: utf-8 -*-
# experiments/study_5_low_rank_tckae_2d/study_5.2_stable_svd/run_sweep.py
#
# --- MODIFIED ---
...
BASE_PERSISTENT_DIR = Path("/cephfs/users/skowronek/Documents/PhD/nuclear_fusion_cooling/prediction/mhd_surrogate_modelling/experiments/study_5_low_rank_tckae_2d/study_5.2_stable_svd/output/test/")

BASE_SCRATCH_DIR = Path("/raid/skowronek/mhd_surrogate_modelling/experiments/study_5_low_rank_tckae_2d/study_5.2_stable_svd/output/test/")

# --- Hyperparameter Grid (MODIFIED for stable loss) ---
...
param_grid = {
    ...
    'gamma_svd_stability': [1e-5], # <-- MODIFIED: Weight for stability (ReLU) loss
    'backward': [False],
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run a hyperparameter sweep for tcKAE (Stable SVD Version).") # <-- MODIFIED
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    logging.info("Starting tcKAE (Stable SVD) hyperparameter sweep...") # <-- MODIFIED
    ...
    for i, config in enumerate(run_configs):
        ...
        run_name_parts.extend([
            ...
            f"gi{config['gamma_identity']}",
            f"gfwd{config['gamma_fwd']}",
            f"g_svdo{config['gamma_svd_ortho']}",
            f"g_svdstab{config['gamma_svd_stability']}", # <-- MODIFIED
        ])
