...
def parse_args():
    ...
    fm_group.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility of noise sampling.")
...
def main():
    ...
    # Set seed for reproducibility (important for FM generation)
    ...
    logging.info(f"Test data ready. Shape: {test_tensor_cpu.shape}. Starting {args.solver} rollout ({args.ode_steps} steps/frame)...")
    ...
    with torch.no_grad():
        for _ in tqdm(range(num_timesteps - 1), desc="Generating frames"):
    ...
    logging.info(f"Rollout complete. Shape: {predicted_timeseries.shape}")
    ...
    np.savez(stats_path,
        ...
        ode_settings=np.array([args.solver, str(args.ode_steps)], dtype='U') # Added this metadata
    )
