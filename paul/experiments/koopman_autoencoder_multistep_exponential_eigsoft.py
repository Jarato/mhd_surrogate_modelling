...
def train_model(kae_model, data_loader, num_epochs, n_steps, alpha, eig_threshold):
    kae_model.train()
    ...
    for epoch in epoch_progress:
        ...
        train_history["loss_list_prediction"][epoch] = prediction_loss_mean
        train_history["loss_list_linearity"][epoch] = linearity_loss_mean
        ...
        epoch_progress.set_description(f"L(recon): {reconstruction_loss_mean:.4f}, L(pred): {prediction_loss_mean:.4f}, L(lin): {linearity_loss_mean:.6f}, Loss(eigV): {eigvalue_loss_mean:.4f}, MaxAbsEigV: {max_abs_eigenvalue:.4f}, StableModes: {sum(mask)}")
...
NUM_STEPS = 5
EIG_THRESHOLD = 0.9
RUN_NAME = f"_L{LATENT_DIMENSION}_a{ALPHA}_e{EPOCHS}_eig{EIG_THRESHOLD}_lr{LR}_st{NUM_STEPS}"
...
##########################################################################################################################################################
    ...
    with open(os.path.join(folder_name, "traceback.txt"), "w+") as tb_file:
        try:
            ...
            loader = DataLoader(dataset, batch_size=5, shuffle=True, num_workers=2, pin_memory=True)
            ...
            trained_model, train_history = train_model(model, loader, EPOCHS, NUM_STEPS, ALPHA, EIG_THRESHOLD)
