...
LATENT_DIMENSION = 128 #[512, 128, 32]
EPOCHS = 300
...
##########################################################################################################################################################
    ...
    with open(os.path.join(folder_name, "traceback.txt"), "w+") as tb_file:
        try:
            ...
            model = ConvAutoencoder(latent_dim=LATENT_DIMENSION, weight_standard=WEIGHTSTD, bias_terms=USEBIAS, norm_type=NORMTYPE, norm_latent=NORMLATENT).to(DEVICE)
