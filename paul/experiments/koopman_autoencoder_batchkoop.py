...
def train_model(kae_model, data_loader, num_epochs):
    ...
    train_history["loss_latent_lin"] = np.zeros((num_epochs))
    ...
    for epoch in epoch_progress:
        ...
        for X_T in data_loader:
            ...
            loss_lin =   mse(Z_pred, Z)

            ### TOTAL LOSS ###
            loss_total = loss_recon + loss_pred + loss_lin
            ...
            linearity_loss_epoch += loss_lin.item() * B
        ...
        latentlin_loss_mean = linearity_loss_epoch / num_train_examples

        total_loss_mean = reconstruction_loss_mean + prediction_loss_mean + latentlin_loss_mean

        train_history["loss_reconstruction"][epoch] = reconstruction_loss_mean
        train_history["loss_prediction"][epoch] = prediction_loss_mean
        train_history["loss_latent_lin"][epoch] = latentlin_loss_mean

        epoch_progress.set_description(f"L(total): {total_loss_mean:.6f}, L(recon): {reconstruction_loss_mean:.6f}, L(pred): {prediction_loss_mean:.6f}, L(latlin): {latentlin_loss_mean:.6f}")
...
LR = 5e-4
WINDOW = 8
RUN_NAME = f"_L{LATENT_DIMENSION}_e{EPOCHS}_lr{LR}_win{WINDOW}_wlin"
...
##########################################################################################################################################################
    ...
    with open(os.path.join(folder_name, "traceback.txt"), "w+") as tb_file:
        try:
            ...
            train_history_keys = ["loss_reconstruction", "loss_prediction", "loss_latent_lin"]
