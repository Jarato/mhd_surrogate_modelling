...
ALPHA = 20
RUN_NAME = "_64_alpha_20_200_small_lr"
...
##########################################################################################################################################################
    ...
    with open(os.path.join(folder_name, "traceback.txt"), "w+") as tb_file:
        try:
            ...
            loader = DataLoader(dataset, batch_size=8, shuffle=True, num_workers=2, pin_memory=True, pin_memory_device=DEVICE)
            ...
            #print(train_history["loss_reconstruction"], train_history["loss_reconstruction"].shape)
            #print(train_history["loss_list_prediction"], train_history["loss_list_prediction"].shape)
            #print(train_history["loss_list_linearity"], train_history["loss_list_linearity"].shape)
            #print(train_history["lin_dyn_weight"], train_history["lin_dyn_weight"].shape)
            ...
            pd.DataFrame(losses_history, columns=["loss_reconstruction"]+[f"loss_prediction_{i}" for i in range(1, NUM_STEPS)]+[f"loss_linearity_{i+1}" for i in range(1, NUM_STEPS)]+["lin_dyn_weight"]).to_csv(os.path.join(folder_name,"losses_history.csv"), index = False)
