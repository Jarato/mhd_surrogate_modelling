...
ALPHA = 20
RUN_NAME = "alpha_20_200_mid_lr_centered_nobias"
...
##########################################################################################################################################################
    ...
    with open(os.path.join(folder_name, "traceback.txt"), "w+") as tb_file:
        try:
            ...
            dataset = TOffsetDataset(data_centered, t_offset=1)
            ...
            model = ConvAutoencoder(latent_dim=LATENT_DIMENSION, use_bias=False).to(DEVICE)
