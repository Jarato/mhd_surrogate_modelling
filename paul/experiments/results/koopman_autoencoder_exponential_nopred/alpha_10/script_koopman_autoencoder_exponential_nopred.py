...
def calculate_dynamic_weight_single(x_t, x_t_reconstructed, alpha = 10):
    with torch.no_grad():
        reconSq_diff_x_t = (x_t.cpu() - x_t_reconstructed.cpu())**2
        recon_mse_example_x_t = torch.mean(reconSq_diff_x_t,dim=[1,2,3])
        dyn_weight_example = np.exp(-alpha*recon_mse_example_x_t.cpu().numpy())
    return dyn_weight_example
...
def train_model(kae_model, data_loader, num_epochs, alpha):
    ...
    for epoch in epoch_progress:
        ...
        for x_t, x_t_plus_1 in data_loader:
            ...
            lin_dyn_weight_epoch += sum(lin_dyn_weight)
            ...
            loss_linearity = torch.mean(torch.mean(error_linearity,dim=[1]) * torch.tensor(lin_dyn_weight).to(DEVICE))

            ### TOTAL LOSS ###
            loss_total = loss_reconstruction + loss_linearity
        ...
        total_loss_mean = reconstruction_loss_mean+linearity_loss_mean
        ...
        epoch_progress.set_description(f"Loss(total): {total_loss_mean:.4f}, Loss(recon): {reconstruction_loss_mean:.4f}, Loss(lin): {linearity_loss_mean:.6f}, MaxAbsEigenV: {max_abs_eigenvalue:.4f}, SteadyModes: {sum(mask)}")
...
##########################################################################################################################################################
    ...
    with open(os.path.join(folder_name, "traceback.txt"), "w+") as tb_file:
        try:
            ...
            losses_history = np.stack([train_history["loss_reconstruction"], train_history["loss_linearity"], train_history["lin_dyn_weight"]], axis=1)
            pd.DataFrame(losses_history, columns=["loss_reconstruction", "loss_linearity", "lin_dyn_weight"]).to_csv(os.path.join(folder_name,"losses_history.csv"), index = False)
