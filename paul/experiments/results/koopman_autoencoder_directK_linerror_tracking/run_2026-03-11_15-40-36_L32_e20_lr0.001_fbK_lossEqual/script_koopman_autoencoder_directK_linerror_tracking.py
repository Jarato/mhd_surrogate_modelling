...
def train_model(model, data_loader, lifting_data_loader, num_epochs):
    ...
    for epoch in epoch_progress:
        ...
        lin_weights = torch.tensor(lin_weights, device=DEVICE, requires_grad=True).real
        print(lin_weights)
        ...
        for x_t, x_t_plus_1 in data_loader:
            ...
            ### TOTAL LOSS ###
            loss_total = reconstruction_weight*loss_reconstruction + linearity_weight*loss_linearity# + loss_prediction 
...
RUN_NAME = f"_L{LATENT_DIMENSION}_e{EPOCHS}_lr{LR}_fbK_lossEqual"
