...
def train_model(kae_model, data_loader, num_epochs, lambda_linear):
    kae_model.train()
    ...
    for epoch in epoch_progress:
        ...
        for x_t, x_t_plus_1 in data_loader:
            ...
            ### TOTAL LOSS ###
            loss_total = loss_reconstruction + loss_prediction + lambda_linear*loss_linearity
...
##########################################################################################################################################################
    ...
    with open(os.path.join(folder_name, "traceback.txt"), "w+") as tb_file:
        try:
            ...
            trained_model, train_history = train_model(model, loader, EPOCHS, lambda_linear)

            torch.save(trained_model.state_dict(), os.path.join(folder_name, "model.pt"))
            
            losses_history = np.stack([train_history["loss_reconstruction"], train_history["loss_prediction"], train_history["loss_linearity"]], axis=1)
            pd.DataFrame(losses_history, columns=["loss_reconstruction", "loss_prediction", "loss_linearity"]).to_csv(os.path.join(folder_name,"losses_history.csv"))

            pd.DataFrame(train_history["eigenvalues"]).to_csv(os.path.join(folder_name,"eigenvalues_history.csv"))
